...
then
    ...
    ($bowtie2bin/bowtie2 -p $cores --dovetail --phred33 --un-gz $aligndir/"$base".unmapped.fq.gz --un-conc-gz $aligndir/"$base".unconcordant.fq.gz -x $bt2idx/genome -1 $trimdir2/"$base"_1.paired.fastq.gz -2 $trimdir2/"$base"_2.paired.fastq.gz) 2> $logdir/"$base".bowtie2 | $samtoolsbin/samtools view -bS - > $aligndir/"$base".bam
else
    ...
    ($bowtie2bin/bowtie2 -p $cores --very-sensitive-local --phred33 --un-gz $aligndir/"$base".unmapped.fq.gz --un-conc-gz $aligndir/"$base".unconcordant.fq.gz -I 10 -X 700 -x $bt2idx/genome -1 $trimdir2/"$base"_1.paired.fastq.gz -2 $trimdir2/"$base"_2.paired.fastq.gz) 2> $logdir/"$base".bowtie2 | $samtoolsbin/samtools view -bS - > $aligndir/"$base".bam
fi
...
#$memebin/meme-chip -oc $meme_outdir -dreme-m $num_motifs -meme-nmotifs $num_motifs $mpaddedfa/$summitfa  modify at 20240411 by ZhenY
$memebin/meme-chip -oc $meme_outdir -meme-nmotifs $num_motifs $mpaddedfa/$summitfa
